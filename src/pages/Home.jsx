...
// Hover wash for the service cards: a pale navy tint that rises from the
  ...
  return (
    <div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8 sm:py-24 lg:px-10">
        ...
        {/* Open cards: just an outline until hovered, when a skyline of flat
            buildings rises into the clear strip under the text (always shown
            on touch screens) */}
        <RevealGroup as="ol" className="mt-12 grid gap-4 sm:mt-14 sm:grid-cols-2 lg:grid-cols-4 lg:gap-5">
          {steps.map((item, i) => (
            <RiseUp
              ...
              key={item.step}
              delay={0.15 + i * 0.15}
              className="group relative isolate flex flex-col overflow-hidden rounded-2xl border border-hairline p-7 pb-36 text-ink transition-colors duration-300 hover:border-accent/25 sm:p-8 sm:pb-40"
            >
          ...
          <RevealGroup as="ul" className="mt-12 grid gap-4 sm:mt-14 lg:grid-cols-3 lg:gap-5">
            {services.map((service, i) => (
              <RiseUp as="li" key={service.title} delay={0.15 + i * 0.15}>
                <Link
                  to="/services"
                  className="group relative isolate flex h-full flex-col overflow-hidden rounded-2xl border border-hairline bg-paper p-6 transition-colors duration-300 hover:border-accent/25 sm:p-8 md:grid md:grid-cols-[auto_1fr_auto] md:items-start md:gap-8 lg:flex lg:min-h-[21rem] lg:items-stretch lg:gap-0"
                >
                  <WaveWash />
